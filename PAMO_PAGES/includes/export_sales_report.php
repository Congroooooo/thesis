...
$sql = "SELECT s.transaction_number, s.item_code, i.item_name, s.size, s.quantity, s.price_per_item, s.total_amount, s.sale_date, 
               s.transaction_type, s.exchange_reference
        FROM sales s
...
$headers = ['Order Number', 'Item Code', 'Item Name', 'Size', 'Quantity', 'Price Per Item', 'Total Amount', 'Sale Date', 'Transaction Type'];
...
logActivity($conn, 'Sales Report Exported', "Sales report exported to Excel with $totalRecords records" . $filterDescription, $user_id);
